...
class Node():
    ...
    def uct(self):
        if self.parent == None:
            return
        self.uct_value = self.score / self.visited \
        + 0.6 * math.sqrt(math.log(self.parent.visited + 1) / self.visited)

    def print(self, a):
        if len(self.children) > 0:
            for c in self.children:
                print(f'{a}{c.name}:{self.children[c].score}:{self.children[c].visited}:{self.children[c].uct_value}')
                self.children[c].print("--" + a)
...
class MCTSPlayer(Player):
    ...
    def get_action(self, cards, actions, dealer_cards):
        ...
        root = Node(None)
        p = RolloutPlayer("Rollout", deck, root)
        ...
        for i in range(MCTS_N):
            ...
            p.currNode.backpropagate(res)
            
                
        act = Action.STAND
        max = -1000000
        for i in root.children:
            if root.children[i].uct_value > max:
                act = i
                max = root.children[i].uct_value
...
class RolloutPlayer(Player):
    ...
    """
    def __init__(self, name, deck, root):
        self.name = name
        self.deck = deck
        self.currNode = root
        self.root = root
    ...
    def get_action(self, cards, actions, dealer_cards):
        ...
        ###################################
        # if len(self.currNode.children) == 0\
        #         or max(self.currNode.children[c].uct_value for c in self.currNode.children) < 0:
        #     act = random.choice(actions)
        #     temp = Node(self.currNode)
        #     self.currNode.children[act] = temp
        #     self.currNode = temp
        #     self.simul = True
        #     return act
        ...
        # act = Action.STAND
        # max_uct = -100000
        # for c in self.currNode.children:
        #     if self.currNode.children[c].uct_value > max_uct:
        #         max_uct = self.currNode.children[c].uct_value
        #         act = c
        # self.currNode = self.currNode.children[act]
        # return act
        ###################################
        ...
        if r < epsilon:
            ...
            if act not in self.currNode.children:
                ...
                child = self.currNode.children[act]
                self.currNode = child
            return act
        else:
            if len(self.currNode.children) == 0:
                ...
                bestAct = Action.STAND
                max = -1000000
                for i in self.currNode.children:
                    if self.currNode.children[i].score / self.currNode.children[i].visited > max:
                        bestAct = i
                        max = self.currNode.children[i].score / self.currNode.children[i].visited
                self.currNode = self.currNode.children[bestAct]
                return bestAct
        ###################################
